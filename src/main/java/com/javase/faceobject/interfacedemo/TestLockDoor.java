...
*       用法：
*           使用interface来修饰
*           接口中可以包含多个方法，且方法跟抽象类的抽象方法一致，可以不写实现
*           子类在实现的时候必须要实现
*           子类实现接口使用implements关键字
*
*       特征：
*           1、接口中的方法都是抽象方法，不能包含方法的实现
*           2、接口中的所有方法访问权限都是public abstract，不写并不是默认的访问权限，而是public
*           3、接口不能被实例化
*           4、接口的子类必须要实现接口中所有方法，跟抽象类有所不同，抽象类中的抽象方法必须要被子类进行实现
*           5、子类可以拥有实现多个接口
*           6、接口中变量都是静态常量,如果变量没有使用static关键字修饰，它也表示静态常量，不用final关键字修饰，也是常量
...
*           2、抽象类需要使用abstract关键字来修饰，而接口使用interface来修饰
*           3、子类使用extends关键字来继承抽象类，使用implements来实现接口
*           4、子类继承抽象类的时候，必须要实现所有的抽象方法，普通方法可以不重写，而接口中所有方法都必须实现
*           5、抽象类中可以定义成员变量，而接口中只能定义静态常量
*           6、抽象类在子类实现的时候是单继承，而接口是多继承
*           7、抽象类和接口都不能实例化，但是抽象类中可以有构造方法，而接口中不能有构造方法
*           8、抽象类中可以继承接口，并且不实现接口中的方法，而接口只能继承接口，不能实现接口
*
*       注意：
*           在实际项目的开发过程中，如果可以使用接口，尽量使用接口，将单继承的父类留在最关键的地方
* */
